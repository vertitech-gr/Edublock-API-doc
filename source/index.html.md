...
#  - go: Go
toc_footers:
  - <a href="https://certs.edu-block.org/">Sign up for an API Key</a>
  - <a href="https://swagger.api.edu-block.org/">Swagger specification</a>
includes: []
search: true
...
# The EduBlock Certs API
<em>A complete solution for creating, managing and presenting Verifiable Credentials.</em>

The EduBlock Certs API allows you to issue, verify and revoke verifiable credentials, manage DIDs and interact with The EduBlock blockchain. Edublock integrates industry-leading World Wide Web Consortium (W3C) and VCDM standards, allowing it to interoperate with other open source technologies. We provide a range of open-source software on [GitHub](https://github.com/vertitech-gr) that can be used alongside the API. Primary features include:
...
- Harness the security of The EduBlock blockchain, a network run by 50 independent validators.
- Work seamlessly across platforms with Edublock standards-compliant, interoperable solutions.

In addition to the code samples shown in these docs, we have provided various code samples for the common requests that you can easily access [here](https://github.com/vertitech-gr/api-samples).

We also offer a free trial, testnet sandboxing and fair monthly pricing. Sign up and [start issuing credentials with Edublock Certs](https://certs.edu-block.org/). Please read our [Terms of Service](https://www.edu-block.org/terms-of-service) before using Edublock Certs.
...
## Prerequisites
You must first have an account and acquire your credentials (API keys) before accessing The EduBlock Certs API. You can register an account and generate a key in your [Edublock Certs](https://certs.edu-block.org/) dashboard.
...
## Endpoints
Dock Certs provides two endpoints based on which mode was selected when creating your API key. By default, the API keys are created for production. You can switch to **test mode** in [Edublock Certs](https://certs.edu-block.org/) by clicking the **test mode** toggle in the top right next to your avatar icon. Once in **test mode** you will see only testnet transactions, API keys, webhooks etc. You can then create an API key from the [Edublock Certs dashboard](https://certs.edu-block.org/keys). It should be noted that in **test mode** your used transaction count **will not increase or hit monthly limits** allowing for sandboxing on our test network.

- For production mode, use the endpoint: [https://api.edu-block.org](https://api.edu-block.org)
- For test mode, use the endpoint: [https://api-testnet.edu-block.org](https://api-testnet.edu-block.org)
...
## Authentication
Dock Certs uses API keys to authenticate requests. You can obtain an API Key by signing into [Edublock Certs](https://certs.edu-block.org). Once a key has been generated, it should be included in **all** request headers as below:
...
## Terminology
It is important to fully understand all the terminologies within The EduBlock ecosystem. The following are common terminologies within our ecosystem:


Terminology | Description
--------- | -----------
DID | DID stands for Decentralized Identifier. It is a new type of identifier that enables verifiable, decentralized digital identity. A DID refers to any subject (e.g., a person, organization, thing, data model, abstract entity, etc.) as determined by the controller of the DID. For more information, please refer [here](https://edublocknetwork.github.io/sdk/tutorials/concepts_did.html).
...
```json
{
  ...
  "data": {
    ...
    "credential": {
      ...
      "id": "https://creds.edu-block.org/f087cbfabc90f8b996971ba47598e82b1a03523cb9460217ad58a819cd9c09eb",
      "type": [
...
Download and use our [Postman Collection](https://github.com/edublocknetwork/api-docs/blob/main/Dock%20API.postman_collection.json) to experiment with basic API flows:

- Download Postman [here](https://www.postman.com/downloads/).
- Download our [API collection here](https://github.com/edublocknetwork/api-docs/blob/main/Dock%20API.postman_collection.json).
...
- Login to [Dock Certs](https://certs.edu-block.org/).
...
- Set `BaseUrl` initial and current values with [https://api-testnet.edu-block.org](https://api-testnet.edu-block.org)
...
The EduBlock API supports `did:dock`, `did:polygonid` and `did:key` method creation.
...
```json
{
    ...
    "id": "https://creds.edu-block.org/presentation/adfb13f1a4b8934d0e94d2aa507e006c",
    "type": [
...
Use our [Swagger UI](https://swagger.api.edu-block.org/) to execute the API calls quickly:
...
For a detailed example of the DIDs workflow. Please refer [here](https://github.com/edublocknetwork/dock-api-js/blob/main/workflows/didFlow.js).
...
```shell
curl --location --request POST 'https://api.edu-block.org/dids' \
...
```shell
curl --location --request GET 'https://api.edu-block.org/dids/did:dock:xyz' \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
- `did:dock:5CEdyZkZnALDdCAp7crTRiaCq6KViprTM6kHUQCD8X6VqGPW` - resolves through The EduBlock blockchain
...
```shell
curl --location --request GET 'https://api.edu-block.org/dids' \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
```shell
curl --location --request DELETE https://api.edu-block.org/dids/{did} \
  --header 'DOCK-API-TOKEN: API_KEY' \
...
```shell
curl --location --request POST https://api.edu-block.org/dids/{did}/export \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw '{
...
```shell
curl --location --request POST 'https://api.edu-block.org/profiles' \
--header 'DOCK-API-TOKEN: API_KEY' \
--data-raw '{
...
```shell
curl --location --request GET 'https://api.edu-block.org/profiles/did:dock:xyz' \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
```shell
curl --location --request GET 'https://api.edu-block.org/profiles' \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
```shell
curl --location --request PATCH 'https://api.edu-block.org/profiles/did:dock:xyz' \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw '{
  "did": "did:dock:xyz",
...
```shell
curl --location --request DELETE https://api.edu-block.org/profiles/{did} \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw '{
    }'
...
The EduBlock API supports issuing two types of credentials. The native Dock Verifiable Credentials and Polygon ID Verifiable Credentials.
...

{
  ...
  "persist": false,
  "distribute": true,
  "recipientEmail": "myemail@edu-block.org",
  "schema": "https://schema.edu-block.org/TestSchema-V1-1695817897561.json",
...
```shell
curl --location --request POST https://api.edu-block.org/credentials/ \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw '{
  "persist": false,
...
For a detailed example of the credential workflow. Please refer [here](https://github.com/edublocknetwork/dock-api-js/blob/main/workflows/credentialsFlow.js).

<h3 id="zero-knowledge-proofs">Zero Knowledge Proofs (ZKP)</h3>

Dock credentials support [anonymous credentials](https://blog.edu-block.org/anonymous-credentials/) using Zero Knowledge Proofs and [Selective Disclosure](https://www.edu-block.org//post/selective-disclosure) by using the BBS+ signing algorithm when issuing the credential. To enable this functionality, simply set the `algorithm` field in the request to `dockbbs+`.
...
```shell
curl --location --request GET 'https://api.edu-block.org/credentials/credential_id?password=test' \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
```shell
curl --location --request DELETE https://api.edu-block.org/credentials/{id} \
  --header 'DOCK-API-TOKEN: API_KEY'
...
```shell
curl --location --request GET 'https://api.edu-block.org/credentials' \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
```
...
[
  {
    "id": "https://creds.edu-block.org/6d601b80d56f4bb2f35b4fbe2406cc186a25b615a66fc405283ad5967f28c143",
...
For a detailed example of the presentations workflow. Please refer [here](https://github.com/edublocknetwork/dock-api-js/blob/main/workflows/presentationsFlow.js).
...
```shell
curl --location --request POST https://api.edu-block.org/presentations/ \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw '{
  "holder": "did:dock:xyz",
...
|domain|body|string|false|A domain for the proof in a string format. The default value for the domain is `edu-block.org`.|
...
```shell
curl --location --request POST https://api.edu-block.org/proof-requests/ \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw '{
...
It often makes sense for a verifier to request proof of credentials from a holder. For this, we have built a proof requests system into the API that works with The EduBlock Wallet. When a request is created, you will receive a URL which you should display in a QR code for a wallet application to scan. You can define which attributes should exist in the credential, a name for the holder and yourself to see and a nonce/challenge which prevents replay attacks.
...
```json
{
  ...
  "presentation": {},
  "response_url": "https://api.edu-block.org/proof-requests/aeec1776-84c3-4783-b80b-c6690a652892/send-presentation",
  "type": "proof-request",
	"qr": "https://creds-testnet.edu-block.org/proof/acaae15e-3c6e-412e-951e-4dc129b9745a",
  "request": {
		"input_descriptors": [
			{
				"id": "ProofIncome-1",
...
```shell
curl --location --request GET https://api.edu-block.org/proof-requests/ \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw'{
...
[
  {
		...
		"presentation": {},
		"response_url": "https://api-testnet.edu-block.org/proof-requests/f2ea3225-ef6b-44d7-a37c-7713c66875b5/send-presentation",
		"type": "proof-request",
		"qr": "https://creds-testnet.edu-block.org/proof/f2ea3225-ef6b-44d7-a37c-7713c66875b5",
		"request": {
			"input_descriptors": [
				{
					"id": "Credential 1",
...
```shell
curl --location --request GET https://api.edu-block.org/proof-requests/{id} \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
```json
{
	...
	"verified": false,
	"response_url": "https://api-testnet.edu-block.org/proof-requests/acaae15e-3c6e-412e-951e-4dc129b9745a/send-presentation",
	"request": {
...
curl -X GET https://api-testnet.edu-block.org/proof-templates \
...
```shell
# You can also use wget
curl -X POST https://api-testnet.edu-block.org/proof-templates \
...
```shell
# You can also use wget
curl -X GET https://api-testnet.edu-block.org/proof-templates/{id}/history \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
```shell
# You can also use wget
curl -X POST https://api-testnet.edu-block.org/proof-templates/{id}/request \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
```shell
# You can also use wget
curl -X GET https://api-testnet.edu-block.org/proof-templates/{id} \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
```shell
# You can also use wget
curl -X PATCH https://api-testnet.edu-block.org/proof-templates/{id} \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
```shell
# You can also use wget
curl -X DELETE https://api-testnet.edu-block.org/proof-templates/{id} \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
For a detailed example of the registry workflow. Please refer [here](https://github.com/edublocknetwork/dock-api-js/blob/main/workflows/registryFlow.js).
...
```shell
curl --location --request POST https://api.edu-block.org/registries/ \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw '{
  "addOnly": true,
...
```shell
curl --location --request GET https://api.edu-block.org/registries/ \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw'{
...
```shell
curl --location --request GET https://api.edu-block.org/registries/{id} \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
```shell
curl --location --request POST https://api.edu-block.org/registries/{id} \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw '{
  "action": "revoke",
  "credentialIds": [
    "https://creds.edu-block.org/f087cbfabc90f8b996971ba47598e82b1a03523cb9460217ad58a819cd9c09eb"
  ]
}'
...
curl --location --request GET https://api.edu-block.org/revocationStatus/{regId}/{revId} \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
```shell
curl --location --request POST https://api.edu-block.org/schemas \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --header 'Content-Type: application/json' \
  --data-raw '{
...
```json
{
  ...
  "data": {
    ...
    "id": "https://schema.edu-block.org/TestSchema-V1-1695817897561.json",
    "uri": "https://schema.edu-block.org/TestSchema-V1-1695817897561.json"
  }
}
```
...
```shell
curl --location --request GET https://api.edu-block.org/schemas \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
```shell
curl --location --request GET https://api.edu-block.org/schemas/{schemaId} \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
Reading a Schema from The EduBlock chain can easily be achieved by using the `get` method which will return the JSON schema to a specific schema ID. The `schemaId`needs to be URL encoded (e.g. `/schemas/https%3A%2F%2Fschema.edu-block.org%2FTestSchema-V1-1695817897561.json`)
...
The EduBlock Blockchain includes a module explicitly intended for proof of existence. You post the hash of a document on-chain at a specific block. Later you can use that hash to prove the document existed at or before that block.

The API allows you to create, get, and retrieve anchors as well as a list of all anchors created by the user.

For a detailed example of the anchor workflow. Please refer [here](https://github.com/edublocknetwork/dock-api-js/blob/main/workflows/anchorsFlow.js).
...
```shell
curl --location --request POST https://api.edu-block.org/anchors \
...
The API will store the `blake2b256` hash of a document or string that you provide. Dock provides a [fully functioning reference client](https://fe.edu-block.org/#/anchor/batch) and [SDK example](https://github.com/edublocknetwork/sdk/blob/master/example/anchor.js) for anchoring.
...
```shell
curl --location --request GET https://api.edu-block.org/anchors \
  --header 'DOCK-API-TOKEN: API_KEY' \
  --data-raw ''
...
[
  {
    ...
    "data": {
      ...
      "documentIds": [
        "https://creds.edu-block.org/credential/b1ed680d3d2d8167dc31bc4913e9c511"
      ]
     },
...
```shell
curl --location --request GET https://api.edu-block.org/anchors/{anchor} \
  --header 'DOCK-API-TOKEN: API_KEY'
  --data-raw ''
...
```shell
curl --location --request GET https://api.edu-block.org/jobs/{id} \
  --header 'DOCK-API-TOKEN: API_KEY'
...
```shell
curl --location --request POST https://api.edu-block.org/verify \
  --header 'DOCK-API-TOKEN: API_KEY'
  --header 'Content-Type: application/json' \
  --data-raw '{
  "@context": [
  ...
  "id": "https://creds.edu-block.org/credential/93a1cd57a46fd5e6e641f0288e2f8b44",
  "type": [
    "VerifiableCredential"
  ],
  "credentialSubject": [
...
curl -X POST https://api-testnet.edu-block.org/subaccounts \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
curl -X GET https://api-testnet.edu-block.org/subaccounts \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
curl -X GET https://api-testnet.edu-block.org/subaccounts/{id} \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
curl -X PATCH https://api-testnet.edu-block.org/subaccounts/{id} \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
curl -X DELETE https://api-testnet.edu-block.org/subaccounts/{id} \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
curl -X GET https://api-testnet.edu-block.org/subaccounts/{id}/usage \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
curl -X POST https://api-testnet.edu-block.org/subaccounts/{id}/keys \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
curl -X GET https://api-testnet.edu-block.org/subaccounts/{id}/keys \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
curl -X DELETE https://api-testnet.edu-block.org/subaccounts/{id}/keys/{keyId} \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
The current most common use case for the messaging service is to send credentials and presentation requests to The EduBlock Wallet, but other clients
can use it too.
...
```shell
# You can also use wget
curl -X POST https://api-testnet.edu-block.org/messaging/encrypt \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
```shell
# You can also use wget
curl -X POST https://api-testnet.edu-block.org/messaging/decrypt \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
```shell
# You can also use wget
curl -X POST https://api-testnet.edu-block.org/messaging/sign \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
```shell
# You can also use wget
curl -X POST https://api-testnet.edu-block.org/messaging/verify \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
```shell
# You can also use wget
curl -X POST https://api-testnet.edu-block.org/messaging/send \
  -H 'Content-Type: application/json' \
  -H 'Accept: application/json' \
  -H 'DOCK-API-TOKEN: API_KEY'
...
{
	"sent": true,
	"qrUrl": "didcomm://https://relay.edu-block.org/read/64502e3243455b67f93f95557"
}
```
...
For a detailed example of the schema workflow. Please refer [here](https://github.com/edublocknetwork/dock-api-js/blob/main/workflows/schemaFlow.js).
...
|Name|Type|Required|Description|
|---|---|---|---|
|id|string(uri)|false|Credential ID. The default value is a creds.edu-block.org uri with random ID.|
...
<script type="application/ld+json">
{
  ...
  "@type": "WebAPI",
  "description": "Edublock provides a complete solution for creating and managing verifiable credentials on the blockchain. This includes a free trial and simple, monthly pricing. Get started here: https://certs.edu-block.org/
",
